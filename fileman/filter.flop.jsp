...
<h3><%=i18n.getText("fileman.filter.flop") %></h3>
<p><%=i18n.getText("fileman.filter.flop.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.flop") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('flop')"><%=i18n.getText("fileman.filter.flop") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Flop filter=new com.laukien.bean.magick.Flop();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.flop.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
