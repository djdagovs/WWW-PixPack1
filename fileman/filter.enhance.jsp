...
<h3><%=i18n.getText("fileman.filter.enhance") %></h3>
<p><%=i18n.getText("fileman.filter.enhance.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.enhance") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('enhance')"><%=i18n.getText("fileman.filter.enhance") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Enhance filter=new com.laukien.bean.magick.Enhance();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.enhance.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
