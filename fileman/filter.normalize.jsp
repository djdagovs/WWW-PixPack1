...
<h3><%=i18n.getText("fileman.filter.normalize") %></h3>
<p><%=i18n.getText("fileman.filter.normalize.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.normalize") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('normalize')"><%=i18n.getText("fileman.filter.normalize") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Normalize filter=new com.laukien.bean.magick.Normalize();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.normalize.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
