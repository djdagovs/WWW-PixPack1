...
<h3><%=i18n.getText("fileman.filter.equalize") %></h3>
<p><%=i18n.getText("fileman.filter.equalize.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.equalize") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('equalize')"><%=i18n.getText("fileman.filter.equalize") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Equalize filter=new com.laukien.bean.magick.Equalize();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.equalize.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
