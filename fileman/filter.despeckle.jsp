...
<h3><%=i18n.getText("fileman.filter.despeckle") %></h3>
<p><%=i18n.getText("fileman.filter.despeckle.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.despeckle") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('despeckle')"><%=i18n.getText("fileman.filter.despeckle") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Despeckle filter=new com.laukien.bean.magick.Despeckle();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.despeckle.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
