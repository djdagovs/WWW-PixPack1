...
<h3><%=i18n.getText("fileman.filter.monochrome") %></h3>
<p><%=i18n.getText("fileman.filter.monochrome.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.monochrome") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('monochrome')"><%=i18n.getText("fileman.filter.monochrome") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Monochrome filter=new com.laukien.bean.magick.Monochrome();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.monochrome.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
