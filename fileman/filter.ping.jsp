...
<h3><%=i18n.getText("fileman.filter.ping") %></h3>
<p><%=i18n.getText("fileman.filter.ping.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.ping") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('ping')"><%=i18n.getText("fileman.filter.ping") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Ping filter=new com.laukien.bean.magick.Ping();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.ping.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
