...
<h3><%=i18n.getText("fileman.filter.flip") %></h3>
<p><%=i18n.getText("fileman.filter.flip.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.flip") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('flip')"><%=i18n.getText("fileman.filter.flip") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Flip filter=new com.laukien.bean.magick.Flip();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.flip.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
