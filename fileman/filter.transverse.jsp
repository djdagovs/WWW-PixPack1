...
<h3><%=i18n.getText("fileman.filter.transverse") %></h3>
<p><%=i18n.getText("fileman.filter.transverse.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.transverse") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('transverse')"><%=i18n.getText("fileman.filter.transverse") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Transverse filter=new com.laukien.bean.magick.Transverse();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.transverse.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
