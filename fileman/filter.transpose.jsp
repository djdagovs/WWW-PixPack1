...
<h3><%=i18n.getText("fileman.filter.transpose") %></h3>
<p><%=i18n.getText("fileman.filter.transpose.text") %></p>
<form onsubmit="return dummy()">
	<table>
		<tr>
			<td style="padding-right: 20px">
				...
				<br/>
				<input type="text" class="field" id="fileman.file" value="<%=file.getName() %>_<%=i18n.getText("fileman.filter.transpose") %>"/>
			</td>
			<td>
	...
	<br/>
	<button onclick="fileman_filter_simple('transpose')"><%=i18n.getText("fileman.filter.transpose") %></button>
</form>
...
<%
		...
		com.laukien.bean.magick.Transpose filter=new com.laukien.bean.magick.Transpose();
		...
		if(filter.isError() && !outFile.exists()) {
			com.enlightware.pixpack.Log.write("filter.transpose.jsp - Filter: "+filter.getRuntimeCommand()+'\n'+filter.getResult(),com.enlightware.pixpack.Log.SYSTEM);
			out.println(i18n.getText("fileman.filter.error"));
			return;
		}
